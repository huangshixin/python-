...
'''读写sv'''
local_file = r'D:\4_data\1_DataSet\watermelon.csv'
data = pd.read_csv(local_file,encoding='gb2312',header=None)#当前方有文字数据时候需要加上 encoding='gb2312'  ,header=None头文件不读
...
def majorityCnt(classList):
    ...
    for vote in classList: #统计classList中每个元素出现的次数
         if vote not in classCount.keys():
             classCount[vote] = 0
             classCount[vote] += 1
             sortedClassCount = sorted(classCount.items(), key = operator.itemgetter(1), reverse = True) #根据字典的值降序排序 return sortedClassCount[0][0] #返回classList中出现次数最多的元素
